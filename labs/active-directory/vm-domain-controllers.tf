...
resource "google_compute_address" "us_east1" {
  name         = "reservation-2"
  address_type = "INTERNAL"
  address      = cidrhost(google_compute_subnetwork.subnet1.ip_cidr_range, var.static_ip)
...
# -------------------------------------------------------------------
# ACTIVE DIRECTORY
# Create the second instance used as the Domain Controller. 
# -------------------------------------------------------------------
...
resource "google_compute_instance" "dc-2" {
  ...
  network_interface {
    network    = google_compute_network.hub.id
    subnetwork = google_compute_subnetwork.subnet1.id
    network_ip = google_compute_address.us_east1.address
    access_config {
      // Ephemeral public IP
    }
  }
